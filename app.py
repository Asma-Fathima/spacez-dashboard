...
# 2. Dataset Ingestion (Bypasses all file reading and memory leaks)
raw_data = [
...
# 4. THE INTERACTIVE AI SANDBOX (Zero-Dependency Code Version)
...
if st.button("Run Real-Time Semantic Analysis"):
    ...
    if any(k in t_input for k in ["clean", "dirty", "sheets", "dishes", "hair"]):
    ...
    elif any(k in t_input for k in ["late", "delay", "unreachable", "wait", "punctual"]):
    ...
    elif any(k in t_input for k in ["photo", "misled", "policy", "gate", "extra"]):
...
# 5. Interactive Cross-Property Cross-Reference Filter
